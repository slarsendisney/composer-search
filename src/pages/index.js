...
const Home = ({ data }) => {
  ...
  console.log(nodes);
  const totalComposers = nodes.reduce((acc, cur) => {
  ...
  return (
    <div className="max-w-4xl text-gray-800 mx-auto p-4 md:py-8">
      <div className="flex-col">
        ...
        <p className="md:my-3">
          👋 I found {totalComposers} matching composer names across {nodes.length} sites. I
          last checked on{" "}
        ...
        {nodes.map(({ composers, website, error }) => (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-800 border-b-2 py-4">
            <a href={website} className="text-blue-600 hover:underline mt-3">
              {website.replace(/(^\w+:|^)\/\//, "")}
            </a>
            {error ? (
